...
class CeskaPostaBalikomatyShipmentExporter implements ShipmentExporterInterface
{
	...
	public function getRow(ShipmentInterface $shipment, array $questionsArray): array
	{
		...
		$balikomat = $shipment->getCeskaPostaBalikomat();
		assert($balikomat !== null);

		assert($balikomat->getZip() !== null);
		$zip = (string) preg_replace('/\s/', '', $balikomat->getZip());
